d3.json('data/twr.json').then((cheok) => {
	...
	const xMin = d3.min(twrCheok, (d) => d.timestamp);
	const xMax = d3.max(twrCheok, (d) => d.timestamp);

	const yMin = d3.min(twrCheok, (d) => d.arrayed);
	const yMax = d3.max(twrCheok, (d) => d.arrayed);
